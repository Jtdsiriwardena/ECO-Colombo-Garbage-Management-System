...
router.post("/", authMiddleware, upload.single("image"), async (req, res) => {
  try {
    ...
    const { location, description, username, area, email } = req.body; 
    
    if (!location || !description || !area) {
      return res.status(400).json({ message: "Location, description, and area are required." });
    }
